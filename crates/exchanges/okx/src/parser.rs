        ...
        while let Some(msg) = self.raw_rx.blocking_recv() {
            match serde_json::from_slice::<WsMessage>(&msg) {
                ...
                Err(_) => {
                    ...
                    error!(exchange = ?Exchange::Okx, component = ?Component::Parser, text = ?text, "error while parsing update");
                },
                        ...
                        DepthBookAction::Snapshot => {
                            for data in depth.data {
                                ...
                                if let Err(e) = self.normalized_tx.blocking_send(event_envelope) {
                                    error!(exchange = ?Exchange::Okx, component = ?Component::Parser, error = ?e, "error while sending snapshot event");
                                }
                            }
                        },
