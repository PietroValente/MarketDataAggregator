...
use crate::types::{ApiResponse, OkxMdMsg, OkxUrls, SubscriptionRequest, SymbolParam};
...
impl OkxConnector {
    pub async fn new(urls: OkxUrls, max_subscription_per_ws: usize, raw_tx: Sender<OkxMdMsg>, control_rx: Receiver<ControlEvent>) -> Result<Self, Box<dyn Error + Send + Sync>> 
    where 
        Self: Sized
    {
        let (inbound_tx, inbound_rx) = channel::<InboundEvent>(4096);
        let ws_url = Arc::from(urls.ws);
        let subscriptions_payloads = Arc::new(OkxConnector::build_subscriptions(&urls.exchange_info, max_subscription_per_ws).await?);
