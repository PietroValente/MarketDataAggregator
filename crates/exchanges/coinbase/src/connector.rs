...
use crate::types::{ApiResponse, CoinbaseConnectorError, CoinbaseMdMsg, CoinbaseUrls, ManagerCommand, SubscriptionMsg, Subscriptions, SymbolParam};
...
impl ExchangeConnector for CoinbaseConnector {
    ...
    async fn build_subscriptions(client: Client, rest_url: &Url, max_subscription_per_ws: usize) -> Result<Subscriptions, Box<dyn Error + Send + Sync + 'static>> {
        if max_subscription_per_ws == 0 {
            return Err(CoinbaseConnectorError::InvalidMaxSubscriptionPerWs.into());
        }
        ...
        match event {
            ControlEvent::Resync => {
                if let Err(e) = manager_tx
                    .send(ManagerCommand::RecreateWithSnapshots)
                    .await {
                        error!(exchange = ?CoinbaseConnector::exchange(), component = ?CoinbaseConnector::component(), error = ?e, "failed to send RecreateWithSnapshots command to manager");
                    }
            }
        }
    }
        ...
        match cmd {
            ...
            ManagerCommand::RecreateWithSnapshots => {
                if recreate_in_progress {
                    info!(exchange = ?CoinbaseConnector::exchange(), component = ?CoinbaseConnector::component(), "recreate already in progress, skipping");
                    continue;
                }
...
async fn pong_ws(connections: &HashMap<u8, ConnectionTasks>, msg: PingMsg) {
    if let Some(conn) = connections.get(&msg.ws_id) {
        ...
        error!(exchange = ?CoinbaseConnector::exchange(), component = ?CoinbaseConnector::component(), "pong requested for unknown connection");
    }
}
        ...
        match recreate_with_snapshots(
            ...
            Ok(()) => {
                if let Err(e) = cmd_tx.send(ManagerCommand::RecreateFinished).await {
                    error!(exchange = ?CoinbaseConnector::exchange(), component = ?CoinbaseConnector::component(), error = ?e, "failed to notify recreate finished");
                }
                return;
            }
            Err(e) => {
                error!(exchange = ?CoinbaseConnector::exchange(), component = ?CoinbaseConnector::component(), error = ?e, "error while recreating connections");
