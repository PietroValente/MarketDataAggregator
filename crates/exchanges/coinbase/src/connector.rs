...
use crate::types::{
    ApiResponse, CoinbaseMdMsg, CoinbaseUrls, SubscriptionMsg, Subscriptions, SymbolParam,
};
...
impl CoinbaseConnector {
        ...
        Self: Sized,
    {
        ...
        let subscriptions = CoinbaseConnector::build_subscriptions(
            client,
            &urls.exchange_info,
    ...
    async fn build_subscriptions(
        ...
        let symbols = retry_with_backoff(
            ...
            |e, attempt: usize, delay: Duration| {
                error!(
                    exchange = ?CoinbaseConnector::exchange(),
                    component = ?CoinbaseConnector::component(),
                    error = ?e,
                    attempt = attempt + 1,
