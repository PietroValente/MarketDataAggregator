...
impl CoinbaseConnector {
    pub async fn new(urls: CoinbaseUrls, max_subscription_per_ws: usize, raw_tx: Sender<CoinbaseMdMsg>, control_rx: Receiver<ControlEvent>) -> Result<Self, Box<dyn Error + Send + Sync>> 
    where 
        Self: Sized
    {
        let (inbound_tx, inbound_rx) = channel::<InboundEvent>(4096);
        let ws_url = Arc::from(urls.ws);
        let subscriptions_payloads = Arc::new(CoinbaseConnector::build_subscriptions(&urls.exchange_info, max_subscription_per_ws).await?);
...
impl ExchangeConnector for CoinbaseConnector {
    ...
    async fn build_subscriptions(rest_url: &Url, max_subscription_per_ws: usize) -> Result<Vec<Message>, Box<dyn Error + Send + Sync>>{
        let mut list = CoinbaseConnector::get_subscriptions_list_backoff(rest_url).await;
        ...
        for i in 0..subscriptions_payloads_len {
            ...
            let product_ids: Vec<String> = list
                .drain(..params_len)
                .collect();
