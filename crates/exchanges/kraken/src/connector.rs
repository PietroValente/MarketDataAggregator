...
use crate::types::{ApiResponse, KrakenMdMsg, KrakenUrls, SubscriptionRequest, SymbolParam};

pub struct KrakenConnector {
    manager_tx: Sender<ManagerCommand>,
    raw_tx: Sender<KrakenMdMsg>,
...
impl KrakenConnector {
    pub async fn new(client:Client, urls: KrakenUrls, max_subscription_per_ws: usize, raw_tx: Sender<KrakenMdMsg>, control_rx: Receiver<ControlEvent>) -> Result<Self, Box<dyn Error + Send + Sync + 'static>> 
    where 
        Self: Sized
    {
        let (inbound_tx, inbound_rx) = channel::<InboundEvent>(4096);
        let ws_url = Arc::from(urls.ws);
        let subscriptions_payloads = Arc::new(KrakenConnector::build_subscriptions(client, &urls.exchange_info, max_subscription_per_ws).await?);
...
impl ExchangeConnector for KrakenConnector {
    ...
    async fn get_subscriptions_list(client:Client, rest_url: &Url) -> Result<Vec<String>, Box<dyn Error + Send + Sync + 'static>> {
        ...
        for (_, symbol_info) in resp.result {
            if symbol_info.status == "online" {
                list.push(symbol_info.wsname);
            }
        }
        Ok(list)
    }
    ...
    async fn build_subscriptions(client:Client, rest_url: &Url, max_subscription_per_ws: usize) -> Result<Vec<Message>, Box<dyn Error + Send + Sync + 'static>>{
        let mut list = KrakenConnector::get_subscriptions_list_backoff(client, rest_url).await;
    ...
    async fn start(&mut self) {
        if let Err(e) = self.subscribe_streams().await {
            error!(exchange = ?Exchange::Kraken, component = ?Component::Connector, error = ?e, "error while subscribing to streams");
            return;
        }
        while let Some(msg) = self.inbound_rx.recv().await {
            match msg {
                InboundEvent::WsMessage(payload) => {
                    if let Err(e) = self.raw_tx.send(KrakenMdMsg(payload)).await {
                        error!(exchange = ?Exchange::Kraken, component = ?Component::Connector, error = ?e, "error while sending the ws message");
                        continue;
                    }
                },
                InboundEvent::Ping(ping_msg) => {
                    if let Err(e) = self.pong(ping_msg).await {
                        error!(exchange = ?Exchange::Kraken, component = ?Component::Connector, error = ?e, "error while sending the pong command");
                        continue;
                    }
                },
                InboundEvent::ConnectionClosed => {
                    info!(exchange = ?Exchange::Kraken, component = ?Component::Connector, "connection close, reconnecting");
                    if let Err(e) = self.subscribe_streams().await {
                        error!(exchange = ?Exchange::Kraken, component = ?Component::Connector, error = ?e, "error while subscribing to streams");
                        continue;
                    }
        ...
        match event {
            ControlEvent::Resync => {
                if let Err(e) = manager_tx
                    .send(ManagerCommand::RecreateWithSnapshots)
                    .await {
                        error!(exchange = ?Exchange::Kraken, component = ?Component::Connector, error = ?e, "failed to send RecreateWithSnapshots command to manager");
                    }
            }
        }
    }
        ...
        match cmd {
            ...
            ManagerCommand::RecreateWithSnapshots => {
                if recreate_in_progress {
                    info!(exchange = ?Exchange::Kraken, component = ?Component::Connector, "recreate already in progress, skipping");
                    continue;
                }
...
async fn pong_ws(connections: &HashMap<u8, ConnectionTasks>, msg: PingMsg) {
    if let Some(conn) = connections.get(&msg.ws_id) {
        ...
        error!(exchange = ?Exchange::Kraken, component = ?Component::Connector, "pong requested for unknown connection");
    }
}
        ...
        match recreate_with_snapshots(
            ...
            Ok(()) => {
                if let Err(e) = cmd_tx.send(ManagerCommand::RecreateFinished).await {
                    error!(exchange = ?Exchange::Kraken, component = ?Component::Connector, error = ?e, "failed to notify recreate finished");
                }
                return;
            }
            Err(e) => {
                error!(exchange = ?Exchange::Kraken, component = ?Component::Connector, error = ?e, "error while recreating connections");
...
async fn recreate_with_snapshots(
    ...
    for (i, message) in subscriptions_payloads.iter().enumerate() {
        ...
        let reader_handle = tokio::spawn(async move {
            KrakenConnector::reader_task(ws_id, writer_url, read, reader_tx_clone).await;
        });

        let writer_handle = tokio::spawn(async move {
            KrakenConnector::writer_task(reader_url, write, writer_rx).await;
        });
