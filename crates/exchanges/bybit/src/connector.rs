        ...
        match event {
            ControlEvent::Resync => {
                if let Err(e) = manager_tx
                    .send(ManagerCommand::RecreateWithSnapshots)
                    .await {
                        error!(exchange = ?BybitConnector::exchange(), component = ?BybitConnector::component(), error = ?e, "failed to send RecreateWithSnapshots command to manager");
                    }
            }
        }
    }
        ...
        match cmd {
            ...
            ManagerCommand::RecreateWithSnapshots => {
                if recreate_in_progress {
                    info!(exchange = ?BybitConnector::exchange(), component = ?BybitConnector::component(), "recreate already in progress, skipping");
                    continue;
                }
...
async fn pong_ws(connections: &HashMap<u8, ConnectionTasks>, msg: PingMsg) {
    if let Some(conn) = connections.get(&msg.ws_id) {
        ...
        error!(exchange = ?BybitConnector::exchange(), component = ?BybitConnector::component(), "pong requested for unknown connection");
    }
}
        ...
        match recreate_with_snapshots(
            ...
            Ok(()) => {
                if let Err(e) = cmd_tx.send(ManagerCommand::RecreateFinished).await {
                    error!(exchange = ?BybitConnector::exchange(), component = ?BybitConnector::component(), error = ?e, "failed to notify recreate finished");
                }
                return;
            }
            Err(e) => {
                error!(exchange = ?BybitConnector::exchange(), component = ?BybitConnector::component(), error = ?e, "error while recreating connections");
