...
impl ExchangeConnector for BybitConnector {
    ...
    async fn start(&mut self) {
        if let Err(e) = self.subscribe_streams().await {
            error!(exchange = ?Exchange::Bybit, component = ?Component::Connector, error = ?e, "error while subscribing to streams");
            return;
        }
            ...
            match msg {
                ...
                InboundEvent::Ping(ping_msg) => {
                    if let Err(e) = self.pong(ping_msg).await {
                        error!(exchange = ?Exchange::Bybit, component = ?Component::Connector, error = ?e, "error while sending the pong command");
                        continue;
                    }
                },
                InboundEvent::ConnectionClosed => {
                    info!(exchange = ?Exchange::Bybit, component = ?Component::Connector, "connection close, reconnecting");
                    if let Err(e) = self.subscribe_streams().await {
                        error!(exchange = ?Exchange::Bybit, component = ?Component::Connector, error = ?e, "error while subscribing to streams");
                        continue;
                    }
        ...
        match event {
            ControlEvent::Resync => {
                if let Err(e) = manager_tx
                    .send(ManagerCommand::RecreateWithSnapshots)
                    .await {
                        error!(exchange = ?Exchange::Bybit, component = ?Component::Connector, error = ?e, "failed to send RecreateWithSnapshots command to manager");
                    }
            }
        }
    }
        ...
        match cmd {
            ...
            ManagerCommand::RecreateWithSnapshots => {
                if recreate_in_progress {
                    info!(exchange = ?Exchange::Bybit, component = ?Component::Connector, "recreate already in progress, skipping");
                    continue;
                }
...
async fn pong_ws(connections: &HashMap<u8, ConnectionTasks>, msg: PingMsg) {
    if let Some(conn) = connections.get(&msg.ws_id) {
        ...
        error!(exchange = ?Exchange::Bybit, component = ?Component::Connector, "pong requested for unknown connection");
    }
}
        ...
        match recreate_with_snapshots(
            ...
            Ok(()) => {
                if let Err(e) = cmd_tx.send(ManagerCommand::RecreateFinished).await {
                    error!(exchange = ?Exchange::Bybit, component = ?Component::Connector, error = ?e, "failed to notify recreate finished");
                }
                return;
            }
            Err(e) => {
                error!(exchange = ?Exchange::Bybit, component = ?Component::Connector, error = ?e, "error while recreating connections");
                let delay = BybitConnector::retry_delay(&backoff_secs, attempt);
...
async fn recreate_with_snapshots(
    ...
    if subscriptions_payloads.len() > BybitConnector::ws_id_capacity() {
        return Err(BybitConnectorError::TooManyWsBatchesForU8Id {
            batches: subscriptions_payloads.len(),
            max_supported: BybitConnector::ws_id_capacity(),
        }
        .into());
    }
    ...
    for (i, message) in subscriptions_payloads.iter().enumerate() {
        ...
        let Some(ws_id) = BybitConnector::ws_id_from_index(i) else {
            return Err(BybitConnectorError::TooManyWsBatchesForU8Id {
                batches: subscriptions_payloads.len(),
                max_supported: BybitConnector::ws_id_capacity(),
            }
        ...
        let reader_handle = tokio::spawn(async move {
            BybitConnector::reader_task(ws_id, writer_url, read, reader_tx_clone).await;
        });

        let writer_handle = tokio::spawn(async move {
            BybitConnector::writer_task(reader_url, write, writer_rx).await;
        });
