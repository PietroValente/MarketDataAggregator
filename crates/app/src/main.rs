...
use binance::{
    ...
    types::{BinanceMdMsg, BinanceUrls},
};
use bitget::{
    adapter::BitgetAdapter,
    connector::BitgetConnector,
    types::{BitgetMdMsg, BitgetUrls},
};
use bybit::{
    adapter::BybitAdapter,
    connector::BybitConnector,
    types::{BybitMdMsg, BybitUrls},
};
use coinbase::{
    adapter::CoinbaseAdapter,
    connector::CoinbaseConnector,
    types::{CoinbaseMdMsg, CoinbaseUrls},
};
use okx::{
    adapter::OkxAdapter,
    connector::OkxConnector,
    types::{OkxMdMsg, OkxUrls},
};

use engine::Engine;
use md_core::{
    events::{ControlEvent, EngineMessage},
    logging::{layer::DbLoggingLayer, writer::DbLoggingWriter},
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _ = tokio::spawn(async move {
        let mut log_writer =
            match DbLoggingWriter::new(&config.scylladb.uri, &config.scylladb.init_path, log_rx)
                .await
            {
                Ok(writer) => writer,
                Err(e) => {
    ...
    /* CONTROL CHANNELS */
    let (binance_control_tx, binance_control_rx) =
        channel::<ControlEvent>(config.channels.control_buffer);
    let (bitget_control_tx, bitget_control_rx) =
        channel::<ControlEvent>(config.channels.control_buffer);
    let (bybit_control_tx, bybit_control_rx) =
        channel::<ControlEvent>(config.channels.control_buffer);
    let (coinbase_control_tx, coinbase_control_rx) =
        channel::<ControlEvent>(config.channels.control_buffer);
    ...
    let (normalized_tx, normalized_rx) =
        channel::<EngineMessage>(config.channels.normalized_buffer);
    ...
    let mut binance_adapter = BinanceAdapter::new(
        binance_raw_tx.clone(),
        binance_raw_rx,
        normalized_tx.clone(),
        binance_control_tx,
    );
    let mut bitget_adapter =
        BitgetAdapter::new(bitget_raw_rx, normalized_tx.clone(), bitget_control_tx);
    let mut bybit_adapter =
        BybitAdapter::new(bybit_raw_rx, normalized_tx.clone(), bybit_control_tx);
    let mut coinbase_adapter =
        CoinbaseAdapter::new(coinbase_raw_rx, normalized_tx.clone(), coinbase_control_tx);
    ...
    tokio::spawn(async move {
        let mut binance_connector = BinanceConnector::new(
            binance_client,
            binance_urls,
            config.binance.max_subscription_per_ws,
            binance_raw_tx,
            binance_control_rx,
        )
        .await
    ...
    tokio::spawn(async move {
        let mut bitget_connector = BitgetConnector::new(
            bitget_client,
            bitget_urls,
            config.bitget.max_subscription_per_ws,
            bitget_raw_tx,
            bitget_control_rx,
        )
        .await
    ...
    tokio::spawn(async move {
        let mut bybit_connector = BybitConnector::new(
            bybit_client,
            bybit_urls,
            config.bybit.max_subscription_per_ws,
            bybit_raw_tx,
            bybit_control_rx,
        )
        .await
    ...
    tokio::spawn(async move {
        let mut coinbase_connector = CoinbaseConnector::new(
            coinbase_client,
            coinbase_urls,
            config.coinbase.max_subscription_per_ws,
            coinbase_raw_tx,
            coinbase_control_rx,
        )
        .await
    ...
    tokio::spawn(async move {
        let mut okx_connector = OkxConnector::new(
            okx_client,
            okx_urls,
            config.okx.max_subscription_per_ws,
            okx_raw_tx,
            okx_control_rx,
        )
        .await
